<form method="GET">
    ...
    <div class="input-group mb-3">
        ...
        <input type="text" name="price_from" placeholder="from" aria-label="from" class="form-control" value="{{ request('price_from') ?? '' }}">
        <input type="text" name="price_to" placeholder="to" aria-label="to" class="form-control" value="{{ request('price_to') ?? '' }}">
    </div>
    ...
    <div class="mb-3">
        ...
        <select class="form-select" name="order_by" aria-label="Default select example">
            @foreach ($orderBy as $value => $name)
                <option
                    {{ $value == request('order_by') ? 'selected' : '' }}
                    value="{{ $value }}"
                >{{ $name }}</option>
            @endforeach
          </select>
